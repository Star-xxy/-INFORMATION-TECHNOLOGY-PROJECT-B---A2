...
class MainWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("动态折线图 - A 和 B 数据")
        self.resize(800, 600)
        len_data = 100
        # 生成两类数据 A 和 B
        self.values_A = [random.randint(50, 100) for _ in range(len_data)]
        self.values_B = [random.randint(0, 60) for _ in range(len_data)]
        # 生成随机数据
        self.start_date = date.today() - timedelta(days=len_data-1)
        self.dates = [self.start_date + timedelta(days=i) for i in range(len_data)]

        self.data_A = list(zip(self.dates, self.values_A))
        self.data_B = list(zip(self.dates, self.values_B))
    ...
    def update_chart(self):
        ...
        # 确保日期一致（按周或按月聚合后，A 和 B 的日期可能不完全一致）
        dates = sorted(set(dates_A).union(set(dates_B)))
        ...
        line_chart = LineChart(
            [("A", values_A_aligned), ("B", values_B_aligned)],
            xaxis_labels=dates,
            chart_title=f"{current_text}聚合数据 - A 和 B",
        )
        self.web.setHtml(line_chart.get_chart().render_embed())
